...
def driver():
    ...
    t = time.time()
    for j in range(50):
      [xstar,ier,its] =  slackerNewton(x0,tol,Nmax,.1)
    elapsed = time.time()-t
    print(evalF(xstar))
...
def evalJ(x): 
    ...
    return J
def finiteDiff(x,h,findex,xindex):
    dx = x
    dx[xindex] += h
    df = (evalF(dx)[findex] - evalF(x)[findex])/h

def evalApproxJ(x,h):
    J = np.array([[finiteDiff(x,h,0,0), finiteDiff(x,h,0,1)],[finiteDiff(x,h,1,0), finiteDiff(x,h,1,1)]])
    return J
...
def slackerNewton(x0,tol,Nmax, checksize):

    ''' Slack Newton = use only the inverse of the Jacobian when x is sufficiently far from last jacobian evaluation'''
    ''' inputs: x0 = initial guess, tol = tolerance, Nmax = max its'''
    ''' Outputs: xstar= approx root, ier = error message, its = num its'''
    xcheck = x0
    J = evalJ(x0)
    Jinv = inv(J)
    for its in range(Nmax):
        if(np.linalg.norm(xcheck-x0) > checksize):
            J = evalJ(x0)
            Jinv = inv(J)
            xcheck = x0
